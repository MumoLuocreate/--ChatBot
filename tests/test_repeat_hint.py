...
def test_a_phrase_she_repeated_in_three_messages_is_reported():
    events = (
        event(1, "qichi", "说好两边一起动，别光我一个人"),
        event(2, "mumo", "好"),
        event(3, "qichi", "记住，两边一起动，谁也别偷懒"),
        event(4, "qichi", "两边一起动这句今晚作数"),
    )

    hint = _recent_repeat_hint(events)

    assert hint is not None
    assert "两边一起动" in hint
    assert "×3" in hint
...
def test_two_occurrences_of_a_short_phrase_stay_silent():
    ...
    events = (
        event(1, "qichi", "说好两边一起动"),
        event(3, "qichi", "两边一起动，别光我一个"),
    )
...
def test_a_long_fragment_needs_three_messages_too():
    ...
    离线扫描（同一天三段真实样本）：去掉例外并把下限降到 4 字后，
    亲密段命中真实模板的轮数 7%→11%，日常段噪声 14%→10%、下午段 17%→0%。
    """

    twice = (
        event(1, "qichi", "别拿旧台词来敲门，门板都认识你了"),
        event(2, "mumo", "唔"),
        event(3, "qichi", "别拿旧台词来敲门，说过这个了"),
    )
    thrice = twice + (event(4, "qichi", "别拿旧台词来敲门这句，我记着呢"),)

    assert _recent_repeat_hint(twice) is None
    hint = _recent_repeat_hint(thrice)
    assert hint is not None
    assert "别拿旧台词来敲门" in hint
    assert "×3" in hint
...
def test_a_four_character_skeleton_is_now_counted():
    ...
    真机那一场里「下面那只手」20 次、「跟着你的节奏走」15 次，提示几乎没提过它们。
    """

    events = (
        event(1, "qichi", "你这一下，我整个人都软了半截"),
        event(2, "mumo", "嗯"),
        event(3, "qichi", "别停，软了半截，我快撑不住了"),
        event(4, "qichi", "你越这样，我越软了半截，接着来"),
    )
...
def test_a_repeat_is_reported_only_on_the_message_that_repeats_it():
    ...
    repeated = (
        event(1, "qichi", "说好两边一起动，谁也别偷懒"),
        event(2, "mumo", "好"),
        event(3, "qichi", "两边一起动，谁也别偷懒，这话我说过"),
        event(4, "qichi", "两边一起动，谁也别偷懒——我数着呢"),
    )
...
def test_only_her_own_words_are_counted():
    ...
    events = tuple(event(index, "mumo", "两边一起动就对了") for index in range(1, 5))
...
def test_the_longest_phrases_win_and_substrings_are_not_repeated_back():
    events = (
        event(1, "qichi", "这条今晚一直作数，你别绕"),
        event(2, "qichi", "这条今晚一直作数，我再说一次"),
        event(3, "qichi", "这条今晚一直作数"),
    )

    hint = _recent_repeat_hint(events)

    assert hint is not None
    assert "这条今晚一直作数" in hint
    # 子串不该在同一个提示里重复列出
    assert hint.count("这条今晚一直作数") == 1


def test_the_hint_states_a_fact_and_does_not_issue_a_word_list():
    events = tuple(event(index, "qichi", "往下走，别停在那儿磨我") for index in range(1, 5))
