...
> 这个仓库**只包含引擎**。角色设定、真实对话、记忆库、部署记录、密钥都不在其中。
...
> **注意**：示例配置里 `net.enabled` 与 `voice.enabled` 默认是 **true**。
...
- 如果某个 key 曾经泄露（进过日志、截图或公开仓库），先去供应商轮换它。
...
代码注释里会看到「历史诊断 §3」「历史实施手册 §2.24」这类标注。它们指代开发期留下的内部设计记录，
**这些记录不在仓库内**——标注只保留当时的判断依据，不对应任何仓库内文件，也不影响任何行为。
